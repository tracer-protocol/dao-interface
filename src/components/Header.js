	...
	return (
		<Layout.Header className={className}>
			<div className="row">
				<div className="col left">
					...
					<StyledMenu
						...
						onSelect={({ key }) => {
							key === '2' ? setSelected([key, '4']) : setSelected([key])
						}}
					>
						<StyledMenuItem key="1">
							<NavLink datatype={selected.includes('1') ? 'SELECTED' : ''} exact to="/">
								Proposals
							</NavLink>
						</StyledMenuItem>
						<StyledMenuItem key="2">
							<NavLink datatype={selected.includes('2') ? 'SELECTED' : ''} to="/allocation/chart">
								Allocation
							</NavLink>
						</StyledMenuItem>
						<StyledMenuItem key="3">
							<NavLink datatype={selected.includes('3') ? 'SELECTED' : ''} to="/dashboard">
								Stake
							</NavLink>
						</StyledMenuItem>
					</StyledMenu>
			...
			<div className="row subMenu" datatype={selected[0] === '2' ? 'DISPLAY' : 'NO_DISPLAY'}>
				<SubMenu theme="dark" mode="horizontal" selectedKeys={selected} onSelect={({ key }) => setSelected(['2', key])}>
					<SubMenuItem key="4">
						<NavLink datatype={selected.includes('4') ? 'SELECTED' : ''} to="/allocation/chart">
							Chart
						</NavLink>
					</SubMenuItem>
					<SubMenuItem key="5">
						<NavLink datatype={selected.includes('5') ? 'SELECTED' : ''} to="/allocation/stakers">
							Stakers
						</NavLink>
					</SubMenuItem>
				</SubMenu>
